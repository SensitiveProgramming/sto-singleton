...
import { Gateway_V1_01 } from "../typechain-types/contracts/gateway/upgradeable/v1.01/Gateway_V1_01";
...
let gatewayV1:Gateway_V1_01;

// Proxy
let stoMatching:STOMatching_V1;
let gw:Gateway_V1_01;
...
describe("Contract Deployment", async function () {
    before(async function() {
        ...
        // 7. Gateway_V1 배포
        const factoryGateway = await ethers.getContractFactory("Gateway_V1_01", {signer: deployer});
        gatewayV1 = await factoryGateway.connect(deployer).deploy("Gateway v1.01");
        ...
        gw = await ethers.getContractAt("Gateway_V1_01", gatewayProxy.target.toString());
    ...
    it("Matching test", async function() {
        ...
        receipt = await (await gw.tokenRegister(isuNo)).wait();
        ...
        // await printReceiptEvent(receipt?.hash, abiPathList, false);
        // console.log(await gw.accountQueryBalance(toBytes(user[0][0]), toBytes(user[0][1])));
        // console.log(await gw.accountQueryBalanceByIsuNo(toBytes(user[0][0]), isuNo, toBytes(user[0][1])));

        receipt = await (await currencyToken.connect(deployer).issue(user[1][2], 10000000)).wait();
        // await printReceiptEvent(receipt?.hash, abiPathList, false);
        // console.log(await currencyToken.balanceOf(user[1][2]));

        receipt = await (await gw.placeBuyOrder(getOrderId(), toBytes(user[1][0]), toBytes(user[1][1]), isuNo, 1000, 50)).wait();
        // await printReceiptEvent(receipt?.hash, abiPathList, false);

        receipt = await (await gw.placeSellOrder(getOrderId(), toBytes(user[0][0]), toBytes(user[0][1]), isuNo, 1000, 60)).wait();
        // await printReceiptEvent(receipt?.hash, abiPathList, false);
